    ...
    D::Doc: Clone,
{
    match term {
        ...
        Term::RecordElim(head, _, name) => (alloc.nil())
            ...
            .append(".")
            .append(name.as_ref()),
        Term::FunctionType(_, param_type_groups, body_type) => pretty_paren(
            alloc,
            prec > Prec::Arrow,
            (alloc.nil())
                .append("Fun")
                ...
                .append(alloc.intersperse(
                    param_type_groups.iter().map(|(param_names, param_type)| {
                        (alloc.nil())
                            ...
                            .append(":")
                            .append(alloc.space())
                            .append(pretty_term_prec(alloc, param_type, Prec::Term))
                            .append(")")
                    }),
                    alloc.space(),
                ))
                ...
                .append(
                    (alloc.nil()).append(alloc.space()).append(
                        pretty_term_prec(alloc, body_type, Prec::Arrow)
                            .group()
                            .nest(4),
                    ),
                ),
        ),
        Term::FunctionArrowType(param_type, body_type) => pretty_paren(
            alloc,
            prec > Prec::Arrow,
            (alloc.nil())
